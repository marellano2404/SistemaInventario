...
namespace Almacen.Core.BL.Almacen.Services 
{
    public class AlmacenServices : IAlmacen
    {
        ...
        #region Entradas de Almacen
        public async Task<List<EntradasAlmacenVM>> GetEntradasdeAlmacen(int estado)
        {
            using (var Conexion = new SqlConnection(Helpers.ContextConfiguration.ConexionString))
            {
                var listaEntradas = new List<EntradasAlmacenVM>();
                try
                {
                    var comando = new SqlCommand();
                    comando.Connection = Conexion;
                    comando.CommandText = "[Almacen].[GetEntradasAlmacen]";
                    comando.CommandType = System.Data.CommandType.StoredProcedure;
                    /*Agregando los parametros*/
                    comando.Parameters.AddWithValue("@Estado", 0);
                    comando.Parameters.AddWithValue("@IdEntrada", null);
                    ...
                    if (Lectura.HasRows)
                    {
                        while (Lectura.Read())
                        {
                            EntradasAlmacenVM resultado = new EntradasAlmacenVM();
                            resultado.MyProperty = Lectura.GetInt32(0);
                            //resultado.IdArticulo = Lectura.GetGuid(1);
                            //resultado.ClaveProducto = Lectura.GetString(2);
                            //resultado.ClaveInventario = Lectura.GetString(3);
                            //resultado.CodigoBarras = Lectura.GetString(4);
